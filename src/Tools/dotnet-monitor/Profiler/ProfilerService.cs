    ...
    internal sealed class ProfilerService
    {
        ...
        public async Task ApplyProfilersAsync(IEndpointInfo endpointInfo, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    // Profiler already applied
                    // Do not apply profiler twice

                    //await _profilerChannel.SendMessage(endpointInfo, new CommandOnlyProfilerMessage(ProfilerCommand.ResetState), cancellationToken);
