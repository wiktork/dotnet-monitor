...
namespace Microsoft.Diagnostics.Monitoring.Tool.UnitTests
{
    ...
    public sealed class CollectionRuleOptionsTests
    {
        ...
        public Task CollectionRuleOptions_CollectTraceAction_InvalidProviderInterval()
        {
            ...
            return ValidateFailure(
                ...
                ex =>
                {
                    string[] failures = ex.Failures.ToArray();
                    Assert.Single(failures);
                    VerifyNestedGlobalInterval(failures, 0);
                });
        }
        ...
        private static void VerifyNestedGlobalInterval(string[] failures, int index)
        {
            ...
            string message = string.Format(CultureInfo.CurrentCulture, Strings.ErrorMessage_NestedValidationError, nameof(WebApi.GlobalCounterOptions), rangeValidationMessage);
            Assert.Equal(message, failures[index]);
        }
    }
}
